...
// Global prisma instance for initial operations only
...
async function loadTranscript(filePath) {
  // Create a local Prisma client for this operation
  ...
  try {
    ...
    const existingEpisode = await localPrisma.episode.findFirst({
      where: {
    ...
    const episode = await localPrisma.episode.create({
      data: {
    ...
    for (const speakerId of speakerIds) {
      await localPrisma.speaker.upsert({
        where: { id: speakerId },
        update: {},
        create: {
          id: speakerId,
    ...
    // 4. Create turns sequentially
    const createdTurns = [];
    for (const turn of turns) {
      const createdTurn = await localPrisma.turn.create({
        data: {
      ...
      createdTurns.push(createdTurn);
    }
    
    // 5. Create segments sequentially
    for (let i = 0; i < segments.length; i++) {
      ...
      if (turnIndex >= 0) {
        ...
        const existingSegment = await localPrisma.segment.findFirst({
          where: {
            episodeId: episode.id,
        ...
        if (!existingSegment) {
          await localPrisma.segment.create({
            data: {
              startTime: segment.startTime,
